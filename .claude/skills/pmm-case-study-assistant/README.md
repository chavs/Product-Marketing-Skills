...
- **Unattended, multi-track background research** — one foundational track establishing the target company's own current value proposition/positioning/messaging (across every capability area the brief touches, not just the newest one), plus one research track per distinct task in the brief (commonly: an analyst-report-driven roadmap ask, a competitive/user-driven roadmap ask, and a content/messaging review ask). All tracks write sourced, tagged findings into one shared local markdown file, and can run overnight since there's no live time pressure.
