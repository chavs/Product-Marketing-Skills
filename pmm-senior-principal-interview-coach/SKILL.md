...
- **Research** — use when the request is about a company or interviewer background for an upcoming PMM interview, at a general level. Search and fetch as needed; ground claims in what was actually found, flag gaps honestly rather than guessing. For a structured, competitor-by-competitor or process-competitor breakdown, use the Competitive intelligence entry point below instead.
- **Competitive intelligence** — use when the request is specifically about competitors: identifying who they are, researching a named product-level competitor in depth, or mapping the end-to-end workaround a persona used before this product existed. Read `References/competitive-intelligence-research.md` before starting this workflow, every time. This is a deeper, multi-step method than general company research above — don't run it automatically as part of a general Research request; only enter it when competitors are the explicit focus.
...
| `/competitors [company or segment]` | Competitive intelligence entry point | Reads `References/competitive-intelligence-research.md` before responding, same as the natural-language trigger. If no argument, ask for company and segment before proceeding (Step 2 of the method). |
